...
region* hierarchicalGrouping(image<rgb> *im, double sigma, double k, double min_size, int colorSpaceIdx, int &num_region, string imgName){
	int width = im->width();
 ...
 //    cout <<  "Save the segments ........." << endl;
	...
	for (int i=0; i<num_combinedRegions; i++) {
		...
		if (i<num_region) {
			...
			// cout << colorHists[i].c1[2] << " " << regionSizes[i] << " " << colorHists[i].normalized(regionSizes[i]).c1[2] << endl;
		...
		// else {
		// 	combinedRegions[i].box[0]=0;
		// 	combinedRegions[i].box[1]=0;
		// 	combinedRegions[i].box[2]=0;
		// 	combinedRegions[i].box[3]=0;
		// 	combinedRegions[i].region_size=0;
		// }
	}
    ...
    for(int i = 0; i < num_combinedRegions; i++) {
        for(int j = i; j < num_combinedRegions; j++){
            if(combinedNeighbors[i*num_combinedRegions+j] == 1){
                ...
                // cout << region_sim[i*num_combinedRegions+j] << endl;
            }
            else{
